<div
  class='px-[clamp(20px,6vw,60px)] md:pr-[20px] h-full flex flex-col lg:hidden  '>
  <p class='text-[1.75rem] pt-5 font-bold '>Newsletter</p>
  <div class='flex w-[280px]'>
    <p class='  border-b-[2px] border-black w-full pb-1 mb-7 mr-4 '>
      Twój e-mail
    </p>
    <div class='mt-[2px]'>
      <svg class='' width="36" height="37" viewBox="0 0 36 37" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M0 22.6032C0 10.6792 10.8009 -0.242055 21.6019 0.00408252C30.3446 0.25022 36 5.71084 36 14.4077C36 26.3317 24.4248 36.7607 13.6239 36.7607C5.14555 36.7607 0 31.0448 0 22.6032Z" fill="#010101"/>
        <g clip-path="url(#clip0_32_978)">
          <path d="M19.7934 11.8578L19.0635 12.5877L24.6106 18.1348L10.0859 18.1348L10.0859 19.164L24.607 19.1676L19.6401 24.1345L20.37 24.8644L26.5849 18.6494L19.7934 11.8578Z" fill="#F7F4F0"/>
        </g>
        <defs>
          <clipPath id="clip0_32_978">
            <rect width="12.0305" height="12.0305" fill="white" transform="translate(18.0781 10.1426) rotate(45)"/>
          </clipPath>
        </defs>
      </svg>
    </div>
  </div>
</div>
