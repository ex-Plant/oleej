<script>
import BigArrowDown from "../../../assets/BigArrowDown.svelte";
</script>

<div
  class="xl:mx-auto grid max-w-[1440px] gap-y-6 px-[clamp(20px,6vw,60px)] py-6 md:gap-y-12 md:pb-12 md:hidden"
>
  <a
    class="{'pr-4 font-bold text-mobile58 md:text-desktop196'}"
    href="mailto:piotrolej@gmail.com"
  >
    <p>NAPISZ</p>
    <span>  DO MNIE </span>
    <div class=" inline-block h-[clamp(24px,7vw,48px)] aspect-square">
      <BigArrowDown />
    </div>
    <p class="h-10 pt-4 text-mobile18 font-bold">piotrolej@gmail.com</p>
  </a>
  <div class="flex space-x-12">
    <p class="text-mobile18 font-bold">SOCIALE</p>
    <div class="grid">
      <a class="h-6 text-mobile16" href="#">olejtopisze</a>
      <a class="h-6 text-mobile16" href="#">kotlet_devolaj</a>
    </div>
  </div>

</div>
